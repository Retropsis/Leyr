...
void UBaseAbilitySystemComponent::AddCharacterCommonAbilities(const TArray<TSubclassOf<UGameplayAbility>>& CommonAbilities)
{
	for (const TSubclassOf<UGameplayAbility> AbilityClass : CommonAbilities)
	{
		FGameplayAbilitySpec AbilitySpec = FGameplayAbilitySpec(AbilityClass, 1);
		// TODO: Common Ability is only HitReact now, not a BaseGameplayAbility but it could change later
		if (const UBaseGameplayAbility* BaseAbility = Cast<UBaseGameplayAbility>(AbilitySpec.Ability))
		{
...
void UBaseAbilitySystemComponent::AbilityInputTagCombo(const FGameplayTag& InputTag)
{
	if (!InputTag.IsValid()) return;
...
void UBaseAbilitySystemComponent::ForEachAbility(const FForEachAbility& Delegate)
{
	...
	for (const FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			UE_LOG(LogLeyr, Error, TEXT("Failed to execute delegate in %hs"), __FUNCTION__);
		}
	}
}
...
bool UBaseAbilitySystemComponent::GetDescriptionsByAbilityTag(const FGameplayTag& AbilityTag, FString& OutDescription, FString& OutNextLevelDescription)
{
	...
	const UAbilityInfo* AbilityInfo = ULeyrAbilitySystemLibrary::GetAbilityInfo(GetAvatarActor());
...
void UBaseAbilitySystemComponent::ClientEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Status, const FGameplayTag& Slot, const FGameplayTag& PreviousSlot) const
{
...
void UBaseAbilitySystemComponent::ServerSpendSkillPoint_Implementation(const FGameplayTag& AbilityTag)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		if (GetAvatarActor()->Implements<UPlayerInterface>())
...
{
	UAbilityInfo* AbilityInfo = ULeyrAbilitySystemLibrary::GetAbilityInfo(GetAvatarActor());
